...
6. **Step 2:页面生成**:
   ...
   - **生命周期写入规则**:
     - 从 Step 1 JSON 的 `entities[].confidence` 和 `connections[].confidence` 映射到 `confidence_score`(见 schema 的映射表)
     - 设置 `last_confirmed` 为当前日期
     - 设置 `evidence_count` 为 Step 1 JSON 中支持此页面的来源数量
     - 检查 `contradictions` 字段,如果有矛盾,填充 `contradicted_by`
     - 从 Step 1 JSON 的 `connections[].type` 提取关系类型,用 `<!-- relation: TYPE -->` 注释写入相关页面区域
     - 对已有页面更新时:如果新信息新于已有信息且内容有变更,递增 `evidence_count` 并更新 `last_confirmed`
...
8. **生成素材摘要页**(`wiki/sources/{日期}-{短标题}.md`):
   ...
   - 对 Step 1 中标记为 `INFERRED` 或 `AMBIGUOUS` 的关系,用 HTML 注释保留置信度和关系类型:
     ```markdown
     <!-- relation: 实现 --><!-- confidence: INFERRED --> [[概念1]]
     <!-- confidence: AMBIGUOUS --> [[概念2]]
     ```
   - frontmatter 必须包含生命周期字段(参考 `templates/source-template.md`):
     ```yaml
     confidence_score: 0.70  # 从 Step 1 confidence 映射
...
9. **更新或创建实体页**(`wiki/entities/`):
   ...
   - 如果已有 → 追加新信息,更新"不同素材中的观点"部分,递增 `evidence_count` 并更新 `last_confirmed`
   - 如果没有 → 创建新实体页,参考 `templates/entity-template.md`
   - frontmatter 必须包含生命周期字段:
     ```yaml
     confidence_score: 0.70  # 从 Step 1 的 entities[].confidence 映射
     last_confirmed: YYYY-MM-DD
     evidence_count: 1
     retention_class: active
     ```
   - 使用 `[[实体名]]` 语法做双向链接
   - 在"相关页面"区域,对 Step 1 JSON 的 `connections` 中涉及本实体的关系,用注释标注关系类型:
     ```markdown
     ## 相关页面
     <!-- relation: 实现 --><!-- confidence: INFERRED --> [[目标实体]]
     ```

10. **更新或创建主题页**(`wiki/topics/`):
   - 识别素材涉及的主要研究主题
   - 如果已有对应主题页 → 更新素材汇总表和核心观点,递增 `evidence_count` 并更新 `last_confirmed`
   - 如果没有 → 创建新主题页,参考 `templates/topic-template.md`
   - frontmatter 必须包含生命周期字段(同实体页)
...
2. **Step 0:调用脚本做机械检查**(必须先做,不要跳过):
   ...
   **Step 0b：生命周期诊断**（如果 lint-runner.sh 未内嵌 lifecycle 检查）：
   ```bash
   node ${SKILL_DIR}/scripts/lifecycle-diagnostics.js "<wiki_root>"
   ```
   将脚本输出的 JSON 读入上下文,作为 Step 3"生命周期诊断"的基础数据。
...
3. **逐项检查**(AI 判断类,脚本做不了):
   ...
   **生命周期诊断**(扫描所有页面的生命周期字段):
   - 统计 retention_class 分布(stable / active / fading / stale / archived)
   - 高亮 `confidence_score < 0.5` 的页面(低置信度,需复核)
   - 高亮 `last_confirmed` 超过 180 天的页面(可能过时)
   - 检查 `contradicted_by` 非空但未设置 `superseded_by` 的页面(矛盾未解决)
   - 检查 `superseded_by` 非空但 `retention_class` 不是 `archived` 的页面(状态不一致)
   - 高亮 `evidence_count = 0` 的页面(无证据支撑)
   - 输出修复建议:对 fading/stale 页面提示用户复核或更新
...
3. **生成结构化深度报告**,保存到 `wiki/synthesis/{主题}-{格式}.md`(按 `WIKI_LANG` 切换语言):
   ...
   **模板 C:时间线格式**(触发词:时间线 / 按时间)
   ...
   - **生命周期写入规则**：digest 生成的 synthesis 页面必须包含 lifecycle frontmatter：
     ```yaml
     confidence_score: 0.50  # INFERRED — 跨素材综合推断
     last_confirmed: YYYY-MM-DD
     evidence_count: {N}  # 综合的素材数量
     retention_class: active
     ```
...
2b. **生成交互式图谱数据**(`wiki/graph-data.json`):
   ...
   解析同行 [[双向链接]] 与 <!-- confidence: ... --> / <!-- relation: TYPE --> 注释，边携带置信度和关系类型
...
**confidence 规则**：
结晶化来源的内容默认标记为 `INFERRED`（来自推断/对话，非原始文档），对应 `confidence_score: 0.50`。

**生命周期写入规则**：
synthesis 页面必须包含以下 frontmatter 字段：
```yaml
confidence_score: 0.50
last_confirmed: YYYY-MM-DD
evidence_count: 1  # 对话本身作为一个证据来源
retention_class: active
```
