    ...
    for (const assignment of assignments) {
      if (assignment.state === "blocked" || meaningful(assignment.blocker)) {
        items.push(inboxItem({
          ...
          detail: assignment.blocker || assignment.nextAction || "Assignment is blocked.",
          mission,
          assignment,
      ...
      if (assignment.state === "review" || assignment.state === "checkpointed") {
        items.push(inboxItem({
          ...
          detail: assignment.result || assignment.nextAction || "Worker output is ready for review.",
          mission,
          assignment,
    ...
    for (const record of missionRunRecords) {
      if (record.type === "approval" && record.state === "pending") {
        items.push(inboxItem({
          ...
          detail: record.summary || record.requester || "Approval is pending.",
          mission,
